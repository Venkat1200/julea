...
static void _benchmark_db_delete(BenchmarkRun *run, gchar const *namespace,
...
static void _benchmark_db_workloadAutoSys(BenchmarkRun *run,
		gchar const *namespace, gboolean use_batch, gboolean use_index_all,
		gboolean use_index_single) {
	gboolean ret;
	g_autoptr (JBatch)
			...
			for (int ii = 0; ii < 10; ii++) {
				{
					...
					*string = _benchmark_db_get_identifier(i);
					g_autoptr (JDBSelector)
					selector = j_db_selector_new(b_scheme,
							J_DB_SELECTOR_MODE_AND, &b_s_error);
					...
					iterator = j_db_iterator_new(b_scheme, selector,
							&b_s_error);
					g_assert_nonnull(iterator);
					g_assert_null(b_s_error);
					...
					ret = j_db_iterator_get_field(iterator, "string",
							&field_type, &field_value, &field_length,
							&b_s_error);
					g_assert_true(ret);
					g_assert_null(b_s_error);
				...
				{
					gint64 i_signed = (((i + N_PRIME) * SIGNED_FACTOR)
					...
					*string = _benchmark_db_get_identifier(i);
					...
					ret = j_db_entry_set_field(entry, "sint", &i_signed, 0,
							&b_s_error);
					g_assert_true(ret);
					g_assert_null(b_s_error);
...
static void benchmark_db_workloadAutoSys(BenchmarkRun *run) {
	_benchmark_db_workloadAutoSys(run, "benchmark_update", false, false, false);
}
static void _benchmark_db_workload_Updates(BenchmarkRun *run,
		gchar const *namespace, gboolean use_batch, gboolean use_index_all,
		gboolean use_index_single) {
	gboolean ret;
	g_autoptr (JBatch)
...
static void benchmark_workload_Updates(BenchmarkRun *run) {
	_benchmark_db_workload_Updates(run, "benchmark_update", false, false, false);
}
...
static void _benchmark_db_workload_Writes(BenchmarkRun *run,
		gchar const *namespace, gboolean use_batch, gboolean use_index_all,
		gboolean use_index_single) {
	gboolean ret;
	g_autoptr (JBatch)
...
static void benchmark_db_insert(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert", false, false, false,
			true);
}

static void benchmark_db_insert_batch(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_batch", true, false,
			false, true);
}

static void benchmark_db_insert_index_single(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_index_single", false,
			false, true, true);
}

static void benchmark_db_insert_batch_index_single(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_batch_index_single", true,
			false, true, true);
}

static void benchmark_db_insert_index_all(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_index_all", false, true,
			false, true);
}

static void benchmark_db_insert_batch_index_all(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_batch_index_all", true,
			true, false, true);
}

static void benchmark_db_insert_index_mixed(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_index_mixed", false, true,
			true, true);
}

static void benchmark_db_insert_batch_index_mixed(BenchmarkRun *run) {
	_benchmark_db_insert(run, NULL, "benchmark_insert_batch_index_mixed", true,
			true, true, true);
}

static void benchmark_db_delete(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete", false, false, false);
}

static void benchmark_db_delete_batch(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_batch", true, false, false);
}

static void benchmark_db_delete_index_single(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_index_single", false, false,
			true);
}

static void benchmark_db_delete_batch_index_single(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_batch_index_single", true,
			false, true);
}

static void benchmark_db_delete_index_all(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_index_all", false, true, false);
}

static void benchmark_db_delete_batch_index_all(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_batch_index_all", true, true,
			false);
}

static void benchmark_db_delete_index_mixed(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_index_mixed", false, true,
			true);
}

static void benchmark_db_delete_batch_index_mixed(BenchmarkRun *run) {
	_benchmark_db_delete(run, "benchmark_delete_batch_index_mixed", true, true,
			true);
}

static void benchmark_db_update(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update", false, false, false);
}

static void benchmark_db_update_batch(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update_batch", true, false, false);
}

static void benchmark_db_update_index_single(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update_index_single", false, false,
			true);
}

static void benchmark_db_update_batch_index_single(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update_batch_index_single", true,
			false, true);
}

static void benchmark_db_update_index_all(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update_index_all", false, true, false);
}

static void benchmark_db_update_batch_index_all(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update_batch_index_all", true, true,
			false);
}

static void benchmark_db_update_index_mixed(BenchmarkRun *run) {
	_benchmark_db_update(run, "benchmark_update_index_mixed", false, true,
			true);
}

static void benchmark_db_update_batch_index_mixed(BenchmarkRun *run) {
